    $(document).ready(function(){
         $("#myTextField").on('keyup', function() { // everytime keyup event
                 ...
                 if ( input.length >= 2 ) { // Minimum characters = 2 (you can change)
                         ...
                         $.ajax({
                                 type: "POST",
                                 url: ROOT_URL + "/product/ajax/autocomplete/update/data", // call the php file ajax/tuto-autocomplete.php (check the routine we defined)
                                 data: data, // Send dataFields var
                                 dataType: 'json', // json method
                                 timeout: 3000,
                                 success: function(response){ // If success
                                         $('#match').html(response.productList); // Return data (UL list) and insert it in the <div id="match"></div>
